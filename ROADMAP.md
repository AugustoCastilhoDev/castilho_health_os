...
Última atualização: 2026-07-28 (PEP: estrutura completa da Memed implementada e verificada ao vivo contra o sandbox real; aguardando apenas as chaves de produção)
...
- **Verificação pendente, só do lado do frontend**: os nomes exatos de evento/comando do widget (`prescricaoImpressa`, `setPaciente`, `newPrescription`, o campo `id`/`prescriptionUuid` no payload do evento) vieram da documentação pública da Memed, não de uma sessão real no navegador contra o widget deles — só o backend (token/cadastro) foi verificado ao vivo até aqui. É preciso reverificar o widget em si (mesmo processo usado pro upload no R2 — Playwright contra o sandbox real) assim que houver como logar como um profissional de verdade pela UI.
...
- Nomes de evento/comando do widget da Memed (`prescricaoImpressa`, `setPaciente`, `newPrescription`) vieram só da documentação pública — o backend (cadastro/token) já foi verificado ao vivo contra o sandbox real, mas o widget em si (frontend, no navegador) ainda não; primeira coisa a validar visualmente.
...
6. ~~PEP — estrutura completa da Memed~~ — feito (2026-07-28): cadastro/token de prescritor, log de auditoria de receita (`MemedPrescriptionLog`), widget no frontend. Backend verificado ao vivo contra o sandbox público da Memed (achou e corrigiu um bug real de formato de payload). Falta só: (a) as chaves de produção da Memed, (b) verificar ao vivo o widget no navegador (frontend), (c) criar o usuário de teste que a Memed exige pra validar a integração.
